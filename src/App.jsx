...
const C = {
  ...
  shadow: '0 2px 10px rgba(22,163,74,0.10)',
};
const inp = {
...
const lbl = {
  display: 'block', fontWeight: 700, fontSize: 12, marginBottom: 5,
  color: C.dark, letterSpacing: 0.3, textTransform: 'uppercase',
};
  ...
  if (page === 'loading') return (
    <div style={{ ...wrap, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontWeight: 800, color: C.primary, marginTop: 12, fontSize: 17 }}>Loading...</p>
      </div>
    </div>
  );
        ...
        {tab === 'signup' && stage === 'form' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div><label style={lbl}>Dukan / Store ka Naam</label>
              <input style={inp} placeholder="Jaise: Kumar Kirana Store" value={f.storeName || ''} onChange={e => set('storeName', e.target.value)} />
            </div>
            <div><label style={lbl}>Poora Address</label>
        ...
        {tab === 'owner' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div><label style={lbl}>Password</label>
              <input style={inp} type="password" placeholder="Password" value={f.password || ''} onChange={e => set('password', e.target.value)} />
            </div>
...
function StockManager({ stock }) {
  ...
  const handleDelete = async id => {
    if (!window.confirm('Yeh product stock se delete karein?')) return;
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <button style={mkBtn('primary')} onClick={openAdd}>+ Product Add Karo</button>
      </div>

      {showForm && (
        <div style={{ ...mkCard, marginBottom: 16, borderColor: C.primary, borderWidth: 2 }}>
          <h3 style={{ margin: '0 0 14px', fontWeight: 800, color: C.dark }}>{editId ? '✏️ Product Edit Karo' : '➕ Naya Product'}</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div><label style={lbl}>Price (₹)</label>
                <input style={inp} type="number" placeholder="85" value={f.price || ''} onChange={e => set('price', e.target.value)} />
              </div>
            ...
            <div key={p.id} style={{ ...mkCard, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: C.muted, marginTop: 3 }}>
                  {fmt(p.price)} / {p.unit}
                  <span style={{ marginLeft: 8, fontWeight: 700, color: p.quantity < 10 ? C.danger : C.primary }}>· Stock: {p.quantity} {p.unit}</span>
                </div>
              </div>
    ...
    <div>
      ...
      {detailOrder && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setDetailOrder(null)}>
          <div style={{ ...mkCard, width: '100%', maxWidth: 420, maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 40, color: C.muted }}>
          ...
          <p style={{ fontWeight: 700 }}>Koi order nahi</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtered.map(o => (
            <div key={o.id} style={{ ...mkCard, padding: '14px 16px', borderLeft: `4px solid ${o.status === 'confirmed' ? C.primary : o.status === 'pending' ? C.accent : C.muted}` }}>
              ...
              {o.deliveryDate && o.status !== 'delivered' && (
                <div style={{ marginTop: 8, fontSize: 13, color: C.primary, fontWeight: 700 }}>📅 Delivery: {fmtDate(o.deliveryDate + 'T00:00:00')}</div>
              )}
            </div>
          ))}
        </div>
...
function CustomerPage({ user, stock, orders, cart, setCart, onLogout }) {
  ...
  return (
    <div style={{ maxWidth: 620, margin: '0 auto', minHeight: '100vh', paddingBottom: 90 }}>
      {/* Header */}
      <div style={{ background: `linear-gradient(135deg,${C.dark} 0%,${C.primary} 100%)`, padding: '16px 16px 0', color: '#fff' }}>
      ...
      {justOrdered && (
        <div style={{ background: C.light, borderBottom: `2px solid ${C.primary}`, padding: '12px 18px', fontWeight: 800, color: C.dark, fontSize: 14 }}>
          🎉 Order place ho gaya! Owner confirm karte hi aapko yahan notification milega.
        </div>
      )}
          ...
          <div style={{ ...mkCard, width: '100%', maxWidth: 620, maxHeight: '92vh', overflowY: 'auto', borderRadius: '20px 20px 0 0', padding: 20 }}>
            ...
            {cart.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '32px 0', color: C.muted }}>
                ...
                <p style={{ fontWeight: 700 }}>Cart khali hai</p>
              </div>
            ) : (
              <>
                {cart.map((item, i) => (
        ...
        {view === 'shop' && (
          <>
            ...
            {stock.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 48, color: C.muted }}>
                <div style={{ fontSize: 48 }}>📦</div>
                <p style={{ fontWeight: 700 }}>Abhi koi product available nahi</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {stock.map(p => {
        ...
        {view === 'myorders' && (
          <>
            ...
            {myOrders.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 48, color: C.muted }}>
                <div style={{ fontSize: 48 }}>📭</div>
                <p style={{ fontWeight: 700 }}>Koi order nahi abhi tak</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {myOrders.map(o => (
                  <div key={o.id} style={{ ...mkCard, borderLeft: `5px solid ${o.status === 'confirmed' ? C.primary : o.status === 'pending' ? C.accent : C.muted}` }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 8 }}>
                      {o.items.map((it, i) => (
                        <span key={i} style={{ background: '#f3f4f6', color: C.muted, padding: '3px 9px', borderRadius: 7, fontSize: 12, fontWeight: 600 }}>{it.name} ×{it.quantity}</span>
                      ))}
                    </div>
                    ...
                    {o.status === 'delivered' && (
                      <div style={{ background: '#f3f4f6', color: C.muted, padding: '9px 12px', borderRadius: 10, fontSize: 13, fontWeight: 700 }}>
                        ✅ Deliver ho gaya — {fmtDate(o.deliveredAt)}
                      </div>
                    )}
                  </div>
                ))}
